        ...
        node->bottom_up(
            [&](const FileTreeNode &node)
            {
                ...
                switch (name_type)
                {
                    ...
                    case NAME_JOLIET:
						{
						char file_name_joliet_[JOLIET_MAX_NAMELEN_RELAXED + 1];
						ckcore::string::utf16_to_ansi(node.file_name_joliet_.c_str(), file_name_joliet_, sizeof(file_name_joliet_));
						ckcore::tstring file_name_joliet_1(file_name_joliet_);
						path.insert(0, ckcore::tstring(ckT("/")) + file_name_joliet_1);
						}						
                        break;
                    default:
                        assert(false);
